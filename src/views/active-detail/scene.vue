<template>
    <section class="resume-index activebg resume-active-app scene-active">
        <app-head title="到现场面试"></app-head>
        <img :src="imageSrc"
             alt=""
             class="resume-img" />
        <section class="record-part">
            <span class="pink-gradient"
                  @click="lookFaceLog">查看我的面试记录&emsp;&gt;</span>
        </section>
        <rule-part :rule="ruleData"
                   :ruleBut="ruleBut"></rule-part>
    </section>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        lookFaceLog() {
            if (os._android()) {
                window.control.facInterviewLog()
            } else if (os._ios()) {
                window.webkit.messageHandlers.facInterviewLog.postMessage('')
            }
        }
    }
}
